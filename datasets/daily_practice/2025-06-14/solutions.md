...
SELECT
  ...
  indep_year,
  2000 - indep_year AS country_age_in_2000,
  CONCAT(SUBSTRING(local_name, 1, 10), '...') AS truncated_local_name
FROM
  countries
WHERE
  indep_year BETWEEN 1901 AND 2000
ORDER BY
  country_age_in_2000;
```
---
...
WITH gnps AS (
  SELECT
    ...
    gnp,
    gnp_old,
    ROUND((gnp - gnp_old) * 100 / gnp_old, 2) AS gnp_growth_percentage
  FROM
    countries
  WHERE
    continent = 'Europe'
    AND (gnp_old IS NOT NULL OR gnp_old != 0)
)
SELECT
  country_name,
  gnp,
  gnp_old,
  gnp_growth_percentage,
  RANK() OVER (ORDER BY gnp_growth_percentage DESC) AS gnp_growth_rank
FROM
  gnps;
```
---
...
### SQL Solution
```sql
WITH avg_populations AS (
  SELECT
    co.code,
    ROUND(SUM(ci.population) / COUNT(ci.name), 2) AS country_avg_city_pop
  FROM
    countries co
  JOIN
...
ranked_cities AS (
  SELECT
    ...
    ap.country_avg_city_pop,
    CASE
      WHEN ci.population > ap.country_avg_city_pop THEN 'Above Average'
      WHEN ci.population < ap.country_avg_city_pop THEN 'Below Average'
      ELSE 'Equal To Average'
    END AS comparison_to_average,
    RANK() OVER (PARTITION BY co.code ORDER BY ci.population DESC) AS population_rank
  FROM
    avg_populations ap
  JOIN
    countries co ON co.code = ap.code
  JOIN
    cities ci ON ci.country_code = co.code
)
SELECT
